...
import { ROUTER_ABI, BOT_ABI } from '../constants';
...
async function someFunction() {
  try {
    ...
    return {
      ...
      MIN_SLIPPAGE_THRESHOLD: config.MIN_SLIPPAGE_THRESHOLD,
      WBNB_DATA: config.WBNB_DATA,
...
class Sandwicher {
  ...
  constructor() {
    ...
    this._provider = new providers.JsonRpcProvider(process.env.JSON_RPC);
  ...
  #monitor = async () => {
    ...
    let wsProvider = new providers.WebSocketProvider(WSS_URL);
  ...
  #process = async (receipt: providers.TransactionResponse) => {
    ...
    try {
      ...
      // );
      // var targetToToken = {}
      // console.log(targetToToken, "targetToTokentargetToToken")
      // console.log(path[path.length - 1],"pajshcjhdskcjhdsjcbjk")
      // console.log(path[path.length - 1].trim().toLowerCase(),"pajshcjhdskcjhdsjcbjk")


      var targetToTokenData: any = config.TOKEN_DATA

      // console.log(targetToTokenData,"targetToTokenDatatargetToTokenData")

      const checkingCoinIsInOurTokenData = targetToTokenData.findIndex((item: { symbol: string, decimals: number, address: string, name: string }) => item?.address?.toLowerCase() === path[path.length - 1].trim().toLowerCase());

      if (checkingCoinIsInOurTokenData === -1) {

        console.info(
          `Skipping: Target is buying with ${path[path.length - 1]}`
        );
        return
      }
      var targetToToken = targetToTokenData[checkingCoinIsInOurTokenData]

      console.log("targetToToken", targetToToken, "targetToToken")


      let targetFromToken = config.WBNB_DATA

      if (
        path[0].trim().toLowerCase() !=
        config.WBNB_ADDRESS.trim().toLowerCase()
      ) {
        console.info(
          `Skipping targetFromToken is not matched:  ${path[0]}`
        );

        return;
      }
      console.log(targetAmountInWei.toNumber(), "targetAmountInWei")
      ...
      console.log(executionPrice.toNumber(), "executionPrice")
      ...
      console.log(targetSlippage, "tar")
      ...
      // console.log(reserveBNB, reserveToken, "reserveBNB, reserveToken ",
      //   parseFloat(
      ...
      var amountIn: any = utils.parseUnits(config.MIN_TARGET_AMOUNT, config.MIN_TARGET_AMOUNT_DECIMAL);
      // console.log(amountIn.toNumber(),tokenBalance.toNumber(), "---------------------------------");
      // if amountIn is greater than token balance, just ignore it
      // if (parseFloat(tokenBalance.toNumber()) > 100) {
      //   console.log(
      ...
      // console.log(buyData, sellData, "data")

      // let { safe, hasTax, buyTax, sellTax, error } = await this.#isSafe({
      //   buyData,
      //   sellData,
      // });
      // console.log(safe, hasTax, buyTax, sellTax, error, "--------------------------------")
      ...
      //   );
      //   sendMessage(`Skipping: Token ${targetToToken.symbol}, ${targetToToken.address
      //     } has a buy tax of ${(buyTax * 100).toFixed(2) + '%'
      //     } and a sell tax of ${(sellTax * 100).toFixed(2) + '%'}`)
      //   return;
      // }
  ...
  prepareBuyAndSellData = (params: {
    ...
    try {
      // Generate the 'buyData' for a buy transaction
      let buyData = utils.defaultAbiCoder.encode(
        ['address', 'uint256', 'uint256', 'address[]'],
        [router, amountIn, amountOutMin, path]
      );

      console.log([router, amountIn, amountOutMin, path], "data from prepareBuyAndSellData")

      // Reverse the 'path' array to generate 'sell_path' for a sell transaction
      let sell_path = [...params.path].reverse();

      // Generate the 'sellData' for a sell transaction
      let sellData = utils.defaultAbiCoder.encode(
        ['address', 'address[]', 'uint256'],
        [router, sell_path, sellAmountOutMin]
      );

      // Return the generated buy and sell data
      return {
        buyData,
        sellData,
      };
    } catch (error: any) {
      // If an error occurs during encoding, throw an error
